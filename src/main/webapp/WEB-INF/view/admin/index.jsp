...
<html>
<head>
<title>EffortLogger - Admin Dashboard</title>
...
<body>
	...
	<section>
		<h3 style="margin-top: 10px">Recent Efforts</h3>
		<table>
			<thead>
				<tr>
					<th>SR</th>
			...
			<tbody>
				<c:if test="${adminEfforts < 3}">
					<c:set var="effortBegin" value="0"></c:set>
					<c:set var="effortEnd" value="${adminEfforts-1}"></c:set>
				</c:if>
				<c:if test="${!(adminEfforts < 3)}">
					<c:set var="effortBegin" value="${adminEfforts-3}"></c:set>
					<c:set var="effortEnd" value="${adminEfforts-1}"></c:set>
				</c:if>
				<c:if test="${adminEfforts eq 0}">
					<tr>
				...
				<c:if test="${!(adminEfforts eq 0)}">
					<c:forEach var="i" begin="${effortBegin}" end="${effortEnd}"
						...
						<tr>
							<td>${ j.count }</td>
							<td>${ recentAdminEfforts[i].effortDate }</td>
							<td>${ recentAdminEfforts[i].effortName }</td>
							<td>${ recentAdminEfforts[i].effortDescription }</td>
							<td>${ recentAdminEfforts[i].effortTimeHours }h${ recentAdminEfforts[i].effortTimeMinutes }m</td>
							<td><c:if
									test="${recentAdminEfforts[i].effortStatus eq 'PENDING'}">
									<span style="color: #FF9800; font-weight: bold"><i
										class="mdi mdi-timer-sand"></i></span>
								</c:if> <c:if
									test="${recentAdminEfforts[i].effortStatus eq 'APPROVED'}">
									<span style="color: #8BC34A; font-weight: bold"><i
										class="mdi mdi-check-circle"></i></span>
								</c:if> <c:if
									test="${recentAdminEfforts[i].effortStatus eq 'REJECTED'}">
	...
	<section>
		...
		<table>
			...
			<tbody>
				<c:if test="${adminBugs < 3}">
					<c:set var="bugBegin" value="0"></c:set>
					<c:set var="bugEnd" value="${adminBugs-1}"></c:set>
				</c:if>
				<c:if test="${!(adminBugs < 3)}">
					<c:set var="bugBegin" value="${adminBugs-3}"></c:set>
					<c:set var="bugEnd" value="${adminBugs-1}"></c:set>
				</c:if>
				<c:if test="${adminBugs eq 0}">
					<tr>
						<td colspan="5" style="color: red; font-weight: bold;">No
							Bugs Reported!</td>
					</tr>
				...
				<c:if test="${!(adminBugs eq 0)}">
					<c:forEach var="i" begin="${bugBegin}" end="${bugEnd}"
						...
						<tr>
							<td>${ j.count }</td>
							<td>${ recentAdminBugs[i].bugDate }</td>
							<td>${ recentAdminBugs[i].bugName }</td>
							<td>${ recentAdminBugs[i].bugDescription }</td>
							<td><c:if
									test="${recentAdminBugs[i].bugStatus eq 'ONGOING'}">
									<span style="color: #FF9800; font-weight: bold"><i
										class="mdi mdi-timer-sand"></i></span>
								</c:if> <c:if test="${recentAdminBugs[i].bugStatus eq 'RESOLVED'}">
									<span style="color: #8BC34A; font-weight: bold"><i
										class="mdi mdi-check-circle"></i></span>
								</c:if> <c:if test="${recentAdminBugs[i].bugStatus eq 'OPEN'}">
